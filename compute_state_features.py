...
def compute_alpha(r, r1, r2):   # r_t, r_t-1, t_t-2
    if r1[1] >= 0:
        alpha1 = -np.arccos(np.dot(np.array([1,0]),r1)/(1 * np.linalg.norm(r1)))
    else:
        alpha1 = np.arccos(np.dot(np.array([1,0]),r1)/(1 * np.linalg.norm(r1)))
    if r2[1] >= 0:
        alpha2 = -np.arccos(np.dot(np.array([1,0]),r2)/(1 * np.linalg.norm(r2)))
    else:
        alpha2 = np.arccos(np.dot(np.array([1,0]),r2)/(1 * np.linalg.norm(r2)))
...
def position(r, r1, p): # inputs must be np.array
    ...
    """
    rel_p = relative_features(p-r, r1-r)
...
def nn_ahead(p, ref_df, last_ref = 0 ):
    j = last_ref
    found = False
    while not found:
        ...
        rel = relative_features(p-r, r1-r)
        rel1 = relative_features(p-r1, r2-r1)
