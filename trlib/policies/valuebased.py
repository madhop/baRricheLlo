...
class Boltzmann(ValueBased):
    """
    The Boltzmann policy.
    Every action is weighted with the Q value.
    The parameter temperature defines the probability of taking a random action.
    """

    def __init__(self,actions,Q,temp):
    ...
    def boltzmann_prob(q_values):
        S = np.sum(np.exp(Q_val/self._temp))# probability denominator: sum(exp(Q(s, a_i)/temp))
        return np.exp(Q_val/self._temp)/S

    def sample_action(self, state):
        Q_val = self._q_values(state)
        p = self.boltzmann_prob(Q_val)
        action_idx = np.random.choice(len(self._actions), 1, p = p)
