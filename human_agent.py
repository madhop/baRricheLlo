...
class HumanAgent(object):
    ...
    def act(self, ob, reward, done, step):
        ...
        if not self.use_logitech_g27:
            ...
            action.append(self.gear(keys[4], keys[5]))
            #action.append(self.gear(rpm, speed_x))
    ...
    def getKeys(self):
        ...
        if keyboard.is_pressed('space') or keyboard.is_pressed('down'):
            ...
            keys.insert(0, 0)
        if keyboard.is_pressed('w'):    # gear up
            keys.append(1)
        else:
            keys.append(0)
        if keyboard.is_pressed('q'):    # gear down
            keys.append(1)
        else:
            keys.append(0)
        return keys
