...
#from policies.cross.rule_based_5w import act_closure
...
class PeRuleBasedPolicy(object):
    """Rule-based policy with Gaussian parameter-based exploration"""
    ...
    def _init(self, ob_space, ac_space,
              means_init=None, logstds_init=None,
              seed=None, verbose=True, n_actions=3, action_closure=None, **kwargs ):
        """Params:
        ...
        """if action_closure is None:
            action_closure = act_closure"""

        self._ob = ob = U.get_placeholder(name="ob", dtype=tf.float32, shape=[None] + list(ob_space.shape))

        if means_init is None:
            means_init = np.zeros(4)
        # Higher order policy (Gaussian)
        with tf.variable_scope('actor') as scope:
            self.actor_weights = tf.get_variable(name='actor_params', shape=[len(means_init)],
                                                             initializer=tf.initializers.constant(means_init))
        ...
        with tf.variable_scope('pol'):
            ...
            if means_init is None:
                means_init = np.zeros(4)
            higher_mean_init =  tf.initializers.constant(means_init, dtype=tf.float32)

            self.higher_mean = higher_mean = tf.get_variable(name='higher_mean', shape=[len(means_init)],
                                                             initializer=higher_mean_init)
            # Diagonal covariance matrix; all stds initialized to

            if logstds_init is None:
                logstds_init = np.ones(4)

            self.higher_logstd = higher_logstd = tf.get_variable(name='higher_logstd',
                                                                     shape=[len(logstds_init)],
                                                                     initializer=tf.initializers.constant(logstds_init))
        ...
        def act(s):
            return action_closure(s, self._get_actor_params)

        self.action = act

        # Act
        self._action = self.action
        self._act = self.action

        # Higher policy weights
        with tf.variable_scope('pol') as scope:
    ...
    class _FrozenLinearActor(object):
        def __init__(self, higher_params, ob_dim, ac_dim):
        ...
        def act(self, ob, resample=False):
            ...
            ob = np.ravel(ob)
            ob = ob.reshape((self.ob_dim, 1))
            theta = self.actor_params.reshape((self.ac_dim, self.ob_dim))
    ...
    def set_weights(self, new_actor_params):
        """Manually set actor policy parameters from flat sequence"""
        self._set_actor_params(new_actor_params)

    def set_theta(self, new_actor_params):
        """Manually set actor policy parameters from flat sequence"""
        self._set_actor_params(new_actor_params)
    ...
    def eval_performance(self, actor_params, rets, behavioral=None):
        ...
        if behavioral is None:
            ...
            return self._get_off_ret_mean(rets, actor_params)[0], self._get_off_ret_std(rets, actor_params, batch_size)[
                0]
    ...
    def eval_bound(self, actor_params, rets, behavioral, rmax, normalize=True,
        ...
        return bound_getter(actor_params, rets, batch_size, ppf, rmax)
    ...
    def _build_iw_graph(self, behavioral):
        ...
        self._get_bound = [U.function(inputs, [bounds[i], renyi]) for i in range(len(bounds))]
        self._get_bound_grad = [U.function(inputs, [bounds[i],
                                                    U.flatgrad(bounds[i], self._higher_params),ret_mean,renyi,batch_size]) for i in
                                range(len(bounds))]
