...
class ParallelSampler(object):
    ...
    def __init__(self, make_pi, make_env, n_workers, horizon, feature_fun, episodes_per_worker=1, gamma=0.999, seed=0):
        ...
        self.workers = [Worker(self.output_queue, self.input_queues[i], self.events[i], make_env, make_pi, self.fun[i], seed + i,i) for i in range(self.n_workers)]
    ...
    def restart(self):
        ...
        for w in self.workers:
            ...
            del w
        self.workers = [
            Worker(self.output_queue, self.input_queues[i], self.events[i], self.make_env, self.make_pi, self.fun[i],
                   self.seed + i, i) for i in range(self.n_workers)]
        for w in self.workers:
            w.start()
