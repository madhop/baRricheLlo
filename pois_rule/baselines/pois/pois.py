...
def traj_segment_generator(pi, env, n_episodes, horizon, stochastic, feature_fun=None):
    ...
    while True:
        ...
        if i == n_episodes:
            yield {"ob" : obs, "rew" : rews, "vpred" : 0, "new" : news,
                    "ac" : acs, "prevac" : prevacs, "nextvpred": 0 * (1 - new),
...
def render(env, pi, horizon, feature_fun=None):
...
def learn(make_env, make_policy, *,
    ...
    if sampler is None:
        seg_gen = traj_segment_generator(pi, env, n_episodes, horizon, stochastic=True, feature_fun=feature_fun)
    ...
    while True:
        ...
        if render_after is not None and iters_so_far % render_after == 0:
            if hasattr(env, 'render'):
                render(env, pi, horizon, feature_fun=feature_fun)
