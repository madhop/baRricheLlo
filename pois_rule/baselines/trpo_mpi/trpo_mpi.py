...
def learn(*,
    ...
    **network_kwargs        keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    losses = [optimgain, meankl, entbonus, surrgain, meanent]#, meanent2, meankl2]
    loss_names = ["optimgain", "meankl", "entloss", "surrgain", "entropy"]#, 'entropy2', "kl2"]
    # losses = [optimgain, meankl, surrgain]
    # loss_names = ["optimgain", "meankl", "surrgain"]
    dist = meankl
    scores = []
    ...
    prob = tf.exp(pi.pd.logp(ac))
    prob_old = tf.exp(oldpi.pd.logp(ac))
    negLogP = U.function([ob, ac], [prob, prob_old])
    ...
    while True:
        ...
        if iters_so_far % checkpoint_freq == 0 and iters_so_far != 0 and checkpoint_dir_out is not None:
            ...
            logger.log('Saved policy weights as %s' %
                       os.path.join(checkpoint_dir_out, 'checkpoint_%d.npy' % iters_so_far))
        ...
        if np.allclose(g, 0):
            ...
            with timed("cg"):
                stepdir = cg(fisher_vector_product, g, cg_iters=cg_iters, verbose=rank == 0 and verbose)
            if np.isfinite(stepdir).all():
