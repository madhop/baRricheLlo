...
Finally, possibles repaires required when the robot crashes or hurts itself of the environment must be taken into account
...
In \cite{formularl} the authors tackle the problem by exploring different formulations of the DDPG algorithm in TORCS.
...
A popular algorithm called DAGGER \cite{dagger} provides a easy way for incorporating human experience. However, a human expert should always be available to provide feedback which is not convenient in
practice. Finn et al. proposed the Inverse Reinforcement
Learning (IRL) \cite{inv}, which automatically learns reward functions from human demonstrations for robotic control. Since it
does not provide a mechanism to obtain feedback from the interaction with the environment, the generalization ability of imitation learning is limited.
...
the simulator (a good result if considered that supervised learning approaches generally yeld to poor performances).
...
This paper \cite{cinesi} aims at improving the achievements of DQfD \cite{dqfd} in integrating reinforcement learning with human demonstrations using TORCS.
...
\begin{itemize}
\item They shift from the discrete action domain contained in \cite{dqfd} to a continuous action domain.
