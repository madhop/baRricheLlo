...
In this framework lives the Policy Optimization via Importance Sampling (POIS) \cite{pois}, which tackles the problem of finding the best policy in a parametric space. There are two versions of the algorithm: Action-based POIS (A-POIS), which is based on a policy gradient approach, and Parameter-based POIS (P-POIS), which adopts the PGPE framework.

In Action-based POIS (A-POIS), the goal is to find a policy that maximizes the performance index $J_D(\theta)\) within a parametric space \(\Pi_\Theta = \{ \pi_\theta:\theta \in \Theta \subseteq \mathbb{R}^p\}\) of stochastic differentiable policies.
