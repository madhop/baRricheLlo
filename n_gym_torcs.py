...
class NTorcsEnv(gym.Env):
    ...
    def __init__(self, reward_function, state_cols, state_bound, ref_df, vision=False, throttle=False,
        ...
        if gear_change:
            self.a_high = np.array([1., 1., 1., 7])
            self.a_low = np.array([-1., 0., 0., 1])
        else:
            self.a_high = np.array([1., 1., 1.])
            self.a_low = np.array([-1., 0., 0.])

        # Actions are scaled in the interval [-1, 1]
        self.action_space = spaces.Box(low=-np.ones(len(self.a_high)), high=np.ones(len(self.a_high)), dtype=np.float32)

        # Create observation space
        state_dim = len(state_cols)
        self.s_low = state_bound['low']
        self.s_high = state_bound['high']

        # States are scaled in the interval [0, 1]
        self.observation_space = spaces.Box(low=np.zeros(len(state_cols)), high=np.ones(len(state_cols)),
                                            dtype=np.float32)
    ...
    def scale_obs(self, obs):
        return (obs - self.s_low) / (self.s_high - self.s_low)

    def unscale_obs(self, scaled_obs):
        return scaled_obs * (self.s_high - self.s_low) + self.s_low

    def scale_action(self, a):
        return 2 * (a - self.a_low) / (self.a_high - self.a_low) - 1.0

    def unscale_action(self, scaled_a):
        return self.a_low + (scaled_a +  1) * (self.a_high - self.a_low) * 0.5
    ...
    def step(self, u, raw=False):
        ...
        if not raw:
            # u is scaled_action in [-1,1] so unscaled it to the TORCS bound
            u = list(self.unscale_action(np.array(u)))
        ...
        if raw:
            ...
            # The state is in the bounds of TORCS, scale it to [0,1]
            self.state = self.scale_obs(self.state)
    ...
    def reset(self, relaunch=False):
        ...
        while auto_drive:
            ...
            action = auto_driver.get_action(track_pos, ob_distFromStart)
            self.observation, _, done, info = self.step(action, raw=True)

            #if self.observation['speed_x'] >= 300:
            if ob_distFromStart < 50:
                # Stop when the agent is near the start
    ...
    def observation_to_state(self, ob, p_1, p_2, prev_action):
        ...
        dir_x = p['x'] - p_1['x']
        dir_y = p['y'] - p_1['x']
        dir_x, dir_y = (dir_x, dir_y) / np.linalg.norm(np.array([dir_x, dir_y]))
